...
function getProvider(): AIProvider {
  ...
  if (provider !== "openai" && provider !== "anthropic" && provider !== "groq") {
    throw new Error(
      `Unsupported AI_PROVIDER: ${provider}. Use "openai", "anthropic", or "groq".`
    );
  }
  return provider;
}
...
function getOpenAICompatibleClient(provider: OpenAICompatibleProvider): OpenAI {
  return provider === "groq" ? getGroqClient() : getOpenAIClient();
}

/**
 * Unified chat completion function that works with OpenAI-compatible providers and Anthropic.
...
export async function chatCompletion(
  ...
  if (provider === "openai" || provider === "groq") {
    return openaiCompletion(messages, tools, model, provider);
  } else {
