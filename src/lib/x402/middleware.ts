...
export function withX402(handler: RouteHandler, options: X402Options): RouteHandler {
  return async (req: NextRequest, context?: unknown) => {
    ...
    const sessionWalletAddress = await getSessionWalletAddress(req).catch(
      () => null
    );
    ...
    if (paymentHeader) {
      ...
      if ((verification.amount ?? 0) < options.priceUsdc) {
        return NextResponse.json(
          {
            success: false,
            error: `Payment amount ${verification.amount} USDC is below the required ${options.priceUsdc} USDC.`,
          },
          { status: 402 }
        );
      }
        ...
        await prisma.payment.create({
          data: {
            userId: await resolveUserId(
              verification.payer || settlement.payer || sessionWalletAddress || "anonymous"
            ),
    ...
    if (sessionWalletAddress) {
      const { allowed } = await checkFreeMessages(sessionWalletAddress);

      if (allowed) {
        // Increment free counter and pass through
        await incrementFreeMessages(sessionWalletAddress);
